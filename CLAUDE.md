...
Versao ativa: **v4.1.58** (backend/PWA — 5 alertas novos: 4 Hidro-Farm + 1 Cam, todos server-only) / **v4.1.47** (firmware nos 4 modulos) — `APP_VERSION` definida como fonte unica em [`server/config.py:24`](./server/config.py). Sync com firmware via [`sync-version.sh`](./sync-version.sh).
