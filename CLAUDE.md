...
Versao ativa: **v4.1.52** (backend/PWA — refator de alertas pra independencia per-modulo, BREAKING) / **v4.1.47** (firmware nos 4 modulos) — `APP_VERSION` definida como fonte unica em [`server/config.py:24`](./server/config.py). Sync com firmware via [`sync-version.sh`](./sync-version.sh).
