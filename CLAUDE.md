...
Versao ativa: **v4.1.54** (backend/PWA — hotfix const reassignment em loadCardHistory) / **v4.1.47** (firmware nos 4 modulos) — `APP_VERSION` definida como fonte unica em [`server/config.py:24`](./server/config.py). Sync com firmware via [`sync-version.sh`](./sync-version.sh).
