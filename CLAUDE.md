...
Versao ativa: **v4.1.53** (backend/PWA — hotfix sqlite3.Row.get() em endpoints novos) / **v4.1.47** (firmware nos 4 modulos) — `APP_VERSION` definida como fonte unica em [`server/config.py:24`](./server/config.py). Sync com firmware via [`sync-version.sh`](./sync-version.sh).
