...
def delete_push_subscription_by_endpoint(endpoint):
    """Remove subscription expirada (sem user_id — chamado quando push falha)."""
    conn = get_db()
...
def get_subscriptions_for_module(chip_id):
    ...
    """, (chip_id,)).fetchall()
    conn.close()
    return [{"endpoint": r["endpoint"], "p256dh": r["p256dh"], "auth": r["auth"], "user_id": r["user_id"]} for r in rows]


# --- Alert Log (cooldown) ---

def should_send_alert(user_id, chip_id, alert_type, cooldown_seconds=3600):
    """Retorna True se nao mandou alerta desse tipo na ultima hora."""
    conn = get_db()
    row = conn.execute(
        "SELECT sent_at FROM alert_log WHERE user_id = ? AND chip_id = ? AND alert_type = ? ORDER BY sent_at DESC LIMIT 1",
