"""
Cultivee — Blueprint Hidro Farm (Premium)
Rotas de controle avancado: status, reles, fases, config. Clone do hidro com capability "hidro-farm".
Registrado em /api/hidro-farm.
"""
...
hidrofarm_bp = Blueprint("hidrofarm", __name__)


def _get_module_or_404(chip_id):
    """Helper: retorna modulo se pertence ao usuario autenticado e tem capability hidro-farm."""
    ...
    try:
        ...
        caps = []
    if "hidro-farm" not in caps:
        return None
    return module


# =====================================================================
# Hidro-Farm routes — montadas em /api/hidro-farm/<chip_id>
# =====================================================================

@hidrofarm_bp.route("/<chip_id>/status")
def status(chip_id):
...
@hidrofarm_bp.route("/<chip_id>/relay")
def relay(chip_id):
...
@hidrofarm_bp.route("/<chip_id>/phases")
def phases(chip_id):
...
@hidrofarm_bp.route("/<chip_id>/save-config", methods=["POST"])
...
@hidrofarm_bp.route("/<chip_id>/add-phase")
...
@hidrofarm_bp.route("/<chip_id>/remove-phase")
...
@hidrofarm_bp.route("/<chip_id>/reset-phases")
...
@hidrofarm_bp.route("/<chip_id>/reset-wifi")
